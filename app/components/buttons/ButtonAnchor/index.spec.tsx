...
import * as stories from '@/components/buttons/ButtonAnchor/index.stories'
import { composeStories } from '@storybook/react'

/** @see https://stackoverflow.com/questions/57827126/how-to-test-anchors-href-with-react-testing-library */
describe('ButtonAnchorコンポーネントの単体テスト', () => {
  const { Default } = composeStories(stories)

  const setup = () => {
    const { user } = render(<Default />)
    const link = screen.getByRole('link', { name: 'ButtonAnchor' })
    return { link, user }
  }
